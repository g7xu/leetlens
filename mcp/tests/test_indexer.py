...
def test_index_carries_full_records(data_repo, records):
    ...
    assert set(index) == {
        "schema_version", "generated_at", "totals", "problems", "sessions",
        "tags", "topics", "daily", "records",
    }
...
def test_index_ranks_topics_as_well_as_tags(data_repo):
    index = build_index(data_repo)
    # Every problem carries LeetCode topics; only some carry user tags.
    assert "dynamic-programming" in index["topics"]
    assert index["topics"]["dynamic-programming"]["give_up_rate"] == 1.0
    assert all("topics" in p for p in index["problems"])
