...
def test_each_url_serves_its_own_repo(hosted, two_repos, records):
    ...
    with pytest.raises(ToolError, match="Set up repo"):
...
def test_store_cache_is_bounded(monkeypatch):
    monkeypatch.setattr(server, "REMOTE_STORE_LIMIT", 2)
    server._remote_stores.clear()
    for n in range(5):
        server._remote_stores[f"o/r{n}"] = object()
        while len(server._remote_stores) > server.REMOTE_STORE_LIMIT:
            server._remote_stores.popitem(last=False)
    assert list(server._remote_stores) == ["o/r3", "o/r4"]
